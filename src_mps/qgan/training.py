...
class Training:
    ...
    def run(self):
        ...
        n = int(np.log2(len(self.final_target_state)))
        with torch.no_grad():
            final_target_state_mps = MPS.from_vector(self.final_target_state.flatten(), [2]*n, normalize=False)
        #final_target_state_mps = MPS([t.detach().requires_grad_() for t in final_target_state_mps._data])
        final_target_state_mps = MPS(final_target_state_mps._data)
        for epoch in range(1, self.config.epochs + 1):
            ...
            for i in range(self.config.iterations_epoch):
                ...
                for _ in range(self.config.steps_dis):
                    ...
                    dis_loss = -1 * compute_cost(self.dis, final_target_state_mps, final_gen_state_detached_mps)
                    dis_loss.backward()
                ...
                if i % self.config.save_fid_and_loss_every_x_iter == 0:
                    ...
                    fid, loss = compute_fidelity_and_cost(self.dis, self.final_target_state, final_gen_state_detached, final_target_state_mps, final_gen_state_detached_mps)
